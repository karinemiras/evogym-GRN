...
from simulation.foraging_ppo import train_ppo_batch
...
class EA(Experiment):
    def __init__(self, args=None):
        ...
        # keep top-N by fitness each generation (0 disables)
    ...
    def run(self):
        ...
        if recovered_population is None:
            ...
            for ind in population:
                ...
                if self.args.run_simulation:
                    prepare_robot_files_online(ind, self.args)

            if self.args.run_simulation:
                train_ppo_batch(population, self.args, self.seed)
        ...
        for generation in range(start_gen, self.num_generations + 1):
            ...
            for _ in range(self.offspring_size):
                ...
                if self.args.run_simulation:
                    prepare_robot_files_online(child, self.args)

            self.update_novelty_archive(offspring)

            if self.args.run_simulation:
                train_ppo_batch(offspring, self.args, self.seed)
